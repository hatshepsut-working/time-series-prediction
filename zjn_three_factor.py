...
if __name__ == "__main__":
    ...
    for mois in csv_file:
        for value in mois:
            #10913
            if len(before_time_series) < 5000:
    ...
    for temp in csv_file:
        for value in temp:
            if len(before_temp_series) < 5000:
                before_temp_series.append(float(value))
    print(len(before_temp_series))
    csv_file = csv.reader(open('air_moisture.csv', 'r'))
    for mois in csv_file:
        for value in mois:
            if(len(before_mois_series)) < 5000:
                before_mois_series.append(float(value))
    print(len(before_mois_series))
    ...
    for i in range(len(before_mois_series)):
        if len(input_mois_series) >= 350:
            break
        if i % 5 == 0 and i != 0:
            five_air.sort()
            input_mois_series.append(five_air[2])
            five_air.clear()
            five_air.append(before_mois_series[i])
        else:
            five_air.append(before_mois_series[i])
    print(input_mois_series)
    print(len(input_mois_series))


    first_factor_time_series = input_time_series
    second_factor_time_series = input_temp_series
    third_factor_time_series = input_mois_series
    ...
    #Third
    x_partition_size = 10
    xmin = math.floor(min(third_factor_time_series) / x_partition_size) * x_partition_size
    xmax = math.ceil(max(third_factor_time_series) / x_partition_size) * x_partition_size
    x_universe = (xmin, xmax)
    print("x_universe:", x_universe)
    ...
    print(v_vectorized)
    #Third
    nIter = int((xmax - xmin) / x_partition_size)
    x_vectorized = []
    for i in range(nIter):
        x_vectorized.append((xmin + i * x_partition_size, xmin + (i + 1) * x_partition_size))
    print(x_vectorized)
    ...
    third_historical_data_fuzzified = []
    for val in third_factor_time_series:
        third_historical_data_fuzzified.append(fetch_fuzzy_class(val, x_vectorized))
    print(third_historical_data_fuzzified)
    ...
    print(len(third_historical_data_fuzzified))
    for i in range(len(first_historical_data_fuzzified) - k):
        ...
        for j in range(k):
            _pair.append((first_historical_data_fuzzified[i + j].get('fuzzy_class'),
                          ...
                          third_historical_data_fuzzified[i + j].get('fuzzy_class')))
    ...
    # Graph Plotting
    plot_comparison_graph(first_historical_data_fuzzified, 'three factors model', ['actual', 'forecasted'])
